...
RSpec.describe "TddGuardRspec::Formatter project root configuration" do
  ...
  describe "env var overrides default" do
    it "writes test.json at TDD_GUARD_PROJECT_ROOT when cwd is within it" do
      RSpec.configuration.tdd_guard_project_root = nil

      Dir.mktmpdir("env-root") do |env_root|
        cwd_dir = File.join(env_root, "sub", "project")
        FileUtils.mkdir_p(cwd_dir)
  ...
  describe "RSpec config overrides env var" do
    it "writes test.json at config path when cwd is within it" do
      Dir.mktmpdir("config-root") do |config_root|
        Dir.mktmpdir("env-root") do |env_root|
          cwd_dir = File.join(config_root, "sub", "project")
          FileUtils.mkdir_p(cwd_dir)
  ...
  describe "path validation" do
    it "falls back to cwd when env var is a relative path" do
      RSpec.configuration.tdd_guard_project_root = nil
      ENV["TDD_GUARD_PROJECT_ROOT"] = "relative/path"

      Dir.mktmpdir do |tmpdir|
        Dir.chdir(tmpdir) do
          run_formatter_and_close

          expected_at_cwd = File.join(tmpdir, test_json_relative_path)
          expect(File.exist?(expected_at_cwd)).to be(true),
            "Expected fallback to cwd: #{expected_at_cwd}"
        end
      end
    end

    it "falls back to cwd when env var points to path where cwd is not within it" do
      RSpec.configuration.tdd_guard_project_root = nil

      Dir.mktmpdir("outside-root") do |outside_root|
        Dir.mktmpdir("cwd-dir") do |cwd_dir|
          ENV["TDD_GUARD_PROJECT_ROOT"] = outside_root

          Dir.chdir(cwd_dir) do
            run_formatter_and_close

            expected_at_cwd = File.join(cwd_dir, test_json_relative_path)
            not_expected_at_root = File.join(outside_root, test_json_relative_path)

            expect(File.exist?(expected_at_cwd)).to be(true),
              "Expected fallback to cwd: #{expected_at_cwd}"
            expect(File.exist?(not_expected_at_root)).to be(false),
              "Should NOT write to root outside cwd: #{not_expected_at_root}"
          end
        end
      end
    end

    it "falls back to cwd when config is a relative path" do
      ENV.delete("TDD_GUARD_PROJECT_ROOT")
      RSpec.configuration.tdd_guard_project_root = "relative/path"
    ...
    it "falls back to cwd when config points to path where cwd is not within it" do
      ENV.delete("TDD_GUARD_PROJECT_ROOT")

      Dir.mktmpdir("outside-root") do |outside_root|
        Dir.mktmpdir("cwd-dir") do |cwd_dir|
          RSpec.configuration.tdd_guard_project_root = outside_root
    ...
    it "falls back to env var when config fails validation but env var is valid" do
      Dir.mktmpdir("env-root") do |env_root|
        Dir.mktmpdir("outside-root") do |outside_root|
          cwd_dir = File.join(env_root, "sub")
          FileUtils.mkdir_p(cwd_dir)
          ...
          Dir.chdir(cwd_dir) do
            ...
            expected_at_env = File.join(env_root, test_json_relative_path)
            not_expected_at_config = File.join(outside_root, test_json_relative_path)

            expect(File.exist?(expected_at_env)).to be(true),
              "Expected fallback to env var: #{expected_at_env}"
            expect(File.exist?(not_expected_at_config)).to be(false),
              "Should NOT write to invalid config path: #{not_expected_at_config}"
          end
        end
      end
    end

    it "falls back to cwd when path does not exist" do
      RSpec.configuration.tdd_guard_project_root = nil
      ENV["TDD_GUARD_PROJECT_ROOT"] = "/nonexistent/path/that/does/not/exist"
    ...
    it "accepts project root when cwd is exactly the root" do
      ENV.delete("TDD_GUARD_PROJECT_ROOT")

      Dir.mktmpdir("exact-root") do |root|
        RSpec.configuration.tdd_guard_project_root = root

        Dir.chdir(root) do
          run_formatter_and_close

          expected_path = File.join(root, test_json_relative_path)
          expect(File.exist?(expected_path)).to be(true),
            "Expected test.json at root: #{expected_path}"
        end
      end
    end
  end
